...
- **Accent** `--color-accent: #c8f24e` (signal lime) · `--color-accent-ink: #4f7300`
...
Lime is the only hue on the site, and it means **status or interaction** —
nothing else:

| Lime | Stays monochrome |
|---|---|
| Status dots (`Available for new work`, portrait `Open`) | All heading text, including the italic accent word |
| Scroll progress bar, About chapter rail | Body copy |
| Active chapter index and marker | Form success / error messages (icons carry the meaning) |
| Hover states — email, links, logo, row hairlines | Buttons and chips at rest |
| Focus ring and `::selection` | Invalid-field borders |
| `Primary practice` badge and card markers | |

Lime is invisible on the cream Projects panel, so touches there use
`accent-ink` — the same hue darkened to hold 5.32:1 on `#fafafa`.

If you want emphasis that is *not* about state, use a contrast step, a hairline,
or italic serif — not more colour.

### Contrast floor

Mid-greys are deliberately brighter than a plain desaturation would produce,
because 11px `.mono-label` text has to clear WCAG AA. Lime is 15.3:1 on the
canvas and `accent-ink` is 5.32:1 on the cream panel. Against `#0a0a0a`:
